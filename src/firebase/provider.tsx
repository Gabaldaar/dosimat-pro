...
export const FirebaseProvider: React.FC<FirebaseProviderProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(
      auth,
      (firebaseUser) => {
        // Limpiar suscripción previa si existe al cambiar de usuario
        if (unsubscribeDoc) {
          unsubscribeDoc();
          unsubscribeDoc = null;
        }
        ...
        if (firebaseUser) {
          ...
          unsubscribeDoc = onSnapshot(doc(firestore, 'users', firebaseUser.uid), (snapshot) => {
            setUserAuthState({
              user: firebaseUser,
          ...
          // Si no hay usuario (logout), limpiar todo el estado
          setUserAuthState({
            user: null,
            userData: null,
            isUserLoading: false,
