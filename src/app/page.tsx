...
export default function Dashboard() {
  ...
  // Queries PROTEGIDAS: Solo se disparan si isStaff es true
  const accountsQuery = useMemoFirebase(() => {
    if (!isStaff) return null;
    return collection(db, 'financial_accounts');
  }, [db, isStaff])

  const txQuery = useMemoFirebase(() => {
    if (!isStaff) return null;
    return query(collection(db, 'transactions'), orderBy('date', 'desc'), limit(50));
  }, [db, isStaff])
  ...
  const routesQuery = useMemoFirebase(() => {
    if (!isStaff) return null;
    return collection(db, 'route_sheets');
  }, [db, isStaff])

  const prodOrdersQuery = useMemoFirebase(() => {
    if (!isStaff) return null;
    return collection(db, 'production_orders');
  }, [db, isStaff])

  const purchOrdersQuery = useMemoFirebase(() => {
    if (!isStaff) return null;
    return collection(db, 'purchase_orders');
  }, [db, isStaff])
  
  // Real-time Data (automáticamente a null si la query es null)
  ...
  // Si no es staff, no dejamos renderizar nada (el useEffect hará el redirect)
  if (isUserLoading || !isStaff) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-background">
        <Loader2 className="h-8 w-8 animate-spin text-primary" />
        <p className="mt-4 text-xs font-bold text-muted-foreground uppercase tracking-widest">Cargando...</p>
      </div>
    )
  }
      ...
      <SidebarInset className="flex-1 w-full pb-48 md:pb-12 p-4 md:p-8 space-y-8 overflow-x-hidden">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64 gap-2">
            <Loader2 className="h-8 w-8 animate-spin text-primary" />
            <p className="text-muted-foreground">Sincronizando...</p>
          </div>
        ) : (
          <>
