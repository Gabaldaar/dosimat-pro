...
function TransactionsContent() {
  ...
  const handleSaveTransaction = () => {
    ...
    if (editingTx) { /* Revertir balances y borrar */ }
    ...
    if (['cobro', 'adjustment', 'Expense'].includes(activeTab)) {
      ...
      const txData = { id: txId, date: finalDateStr, clientId: selectedCustomerId || null, type: activeTab, amount: finalAmount, currency: manualCurrency, description: txDescription || `${activeTab} manual`, financialAccountId: manualAccountId === "pending" ? null : manualAccountId, paidAmount: activeTab === 'cobro' ? finalAmount : 0, pendingAmount: (activeTab === 'adjustment' && finalAmount < 0) ? finalAmount : 0 }
      ...
      ['ARS', 'USD'].forEach(curr => {
        ...
        const txId = Math.random().toString(36).substring(2, 11);
        const txData = { id: txId, date: finalDateStr, clientId: selectedCustomerId || null, type: activeTab, amount: -Number(total), paidAmount: paid, debtAmount: debt, currency: curr, description: txDescription || `Operación ${activeTab}`, financialAccountId: (destinationAccounts[curr]==="pending" || paid === 0) ? null : destinationAccounts[curr], items: selectedItems.filter(i => i.currency === curr), pendingAmount: -debt }
        setDocumentNonBlocking(doc(db, 'transactions', txId), txData, { merge: true })
  ...
  const confirmDeleteTx = () => { /* Lógica de borrado */ }

  const handleOpenWsDialog = (tx: any) => {
    setSelectedTxForWs(tx); setIsWsDialogOpen(true);
  }

  const handleSendWs = () => { /* Lógica de envío */ }

  const allDynamicFieldsFilled = true;
                      ...
                      <TabsList className="grid grid-cols-5 w-full h-auto p-1 bg-muted/50 border">
                          {['sale', 'refill', 'service', 'cobro', 'adjustment'].map(key => (
                            <TabsTrigger key={key} value={key} className="data-[state=active]:bg-primary data-[state=active]:text-white py-2 flex flex-col gap-1">
                              {/* @ts-ignore */}
                              {txTypeMap[key].icon({ className: "h-4 w-4" })}
                      ...
                      <Table>
                        <TableHeader className="bg-muted/30">
                          <TableRow>
                            ...
                            <TableHead className="w-36 text-center">Precio Unit.</TableHead>
                        ...
                        <TableBody>
                          {selectedItems.map((item, i) => (
                            <TableRow key={i}>
                              ...
                              <TableCell><Input type="number" value={item.qty} className="h-8 text-center font-bold" onChange={(e) => { const n = [...selectedItems]; n[i].qty = Number(e.target.value); setSelectedItems(n); }} /></TableCell>
                              <TableCell><Input type="number" value={item.price} className="h-8 text-center font-bold" onChange={(e) => { const n = [...selectedItems]; n[i].price = Number(e.target.value); setSelectedItems(n); }} /></TableCell>
                              <TableCell><Input type="number" value={item.discount} className="h-8 text-center font-bold" onChange={(e) => { const n = [...selectedItems]; n[i].discount = Number(e.target.value); setSelectedItems(n); }} /></TableCell>
                              <TableCell>
                                <Tabs value={item.currency} onValueChange={(v) => { const n = [...selectedItems]; n[i].currency = v; setSelectedItems(n); }} className="w-full">
                                  <TabsList className="grid grid-cols-2 h-8 p-0.5 border">
                                    <TabsTrigger value="ARS" className="text-[8px] font-black data-[state=active]:bg-blue-600 data-[state=active]:text-white">ARS</TabsTrigger>
                                    <TabsTrigger value="USD" className="text-[8px] font-black data-[state=active]:bg-emerald-600 data-[state=active]:text-white">USD</TabsTrigger>
                                  </TabsList>
                                </Tabs>
                              </TableCell>
                              <TableCell className="text-right font-black text-xs">{(item.price * item.qty * (1 - item.discount/100)).toLocaleString()}</TableCell>
                              <TableCell><Button variant="ghost" size="icon" onClick={() => setSelectedItems(selectedItems.filter((_, idx) => idx !== i))}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                    ...
                    {['ARS', 'USD'].map(curr => {
                      ...
                      return (
                        <div key={curr} className="p-4 rounded-xl border bg-muted/10 space-y-3">
                          ...
                          {debt > 0 && (
                            <div className="pt-2 border-t border-dashed mt-2 text-rose-600 font-bold text-xs flex justify-between uppercase">
