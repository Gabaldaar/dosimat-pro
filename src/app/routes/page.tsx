  ...
  const renderSheetList = (sheets: any[]) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {sheets.map((sheet: any) => {
        ...
        const hasPendingSettlements = isCompleted && sheet.items?.some((i: any) => !i.liquidadoRepositor || !i.liquidadoComunicador);
        return (
              ...
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-2">
                  ...
                  {hasPendingSettlements && (
                    <Badge variant="outline" className="text-[10px] font-black uppercase tracking-wider w-fit bg-amber-50 text-amber-700 border-amber-200">
                      <AlertTriangle className="h-3 w-3 mr-1" /> Liq. Pendiente
                    </Badge>
                  )}
                </div>
                {(isAdmin || (isCommunicator && sheet.status === 'planned')) && !isCompleted && (<Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10" onClick={(e) => { e.stopPropagation(); setSheetToDelete(sheet); }}><Trash2 className="h-4 w-4" /></Button>)}
              </div>
